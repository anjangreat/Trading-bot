...
class ConTrader(tpqoa.tpqoa):
    def __init__(self, conf_file, instrument, bar_length, window, units, sl_perc = None, tsl_perc = None, tp_perc = 0.0001):
    ...
    def check_positions(self): 
        exp_position = self.position*self.units 
        ...
        if actual_position != exp_position: 
            self.position = actual_position / self.units 
            try:
                latest_actions = self.get_transactions(self.order_id) 
                for action in latest_actions:
                    if action["type"] == "ORDER_FILL": 
                        self.report_trade(action, "GOING NEUTRAL") 
            except:
                pass
            finally:
                self.terminate_session("SL/TP Event!") 
        elif self.position != self.data["position"].iloc[-1]:
            self.execute_trades()
        else: 
            pass
